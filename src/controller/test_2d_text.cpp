...
void test_2d_text::loop(dfw::input& input, const dfw::loop_iteration_data& lid) {
	...
	switch(state) {
		...
		case tstates::dictate:
			...
			lm::log(s_resources.get_log(), lm::lvl::debug)<<"into dictate state"<<std::endl;
		...
		case tstates::wait:
			...
			lm::log(s_resources.get_log(), lm::lvl::debug)<<"into wait state"<<std::endl;
			...
			if(input.is_input_down(input::activate)) {
				...
				if(!text_buffer.size()) {
					...
					lm::log(s_resources.get_log(), lm::lvl::debug)<<"end of buffer"<<std::endl;
				
					set_state(state_test_2d);
				}
				else {
				
					lm::log(s_resources.get_log(), lm::lvl::debug)<<"back to dictate state"<<std::endl;
...
void test_2d_text::receive(const dfw::broadcast_signal& signal) {
	...
	lm::log(s_resources.get_log(), lm::lvl::debug)<<"got signal into test_2d_test: "<<std::endl;

	switch(signal.get_type()) {
	
		case t_signal_text_display:
		
			lm::log(s_resources.get_log(), lm::lvl::debug)<<"it is a text display signal"<<std::endl;	
			text_buffer=tools::explode(static_cast<const signal_text_display&>(signal).text, "[@np]");
			
			lm::log(s_resources.get_log(), lm::lvl::debug)<<"text is '"<<static_cast<const signal_text_display&>(signal).text<<"'"<<std::endl;	
		...
		case t_signal_reset_state:
			...
			lm::log(s_resources.get_log(), lm::lvl::debug)<<"it is a text reset signal"<<std::endl;
