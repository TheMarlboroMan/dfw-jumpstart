...
struct room_drawable_collection
{
	...
	std::vector<const app::drawable *>	background; //These are not ordered.
	std::vector<const app::drawable *>	main;	//The must be ordered.
	std::vector<const app::drawable *>	foreground; //These are not ordered.
};
...
class room
{
	...
	std::vector<const app::spatiable *>	get_walls_by_box(const tbox&) const;
	std::vector<const app::spatiable *> get_obstacles() const;
	std::vector<object_audio_player>&	get_audio_players() {return audio_players;}
	void				inject_audio_dispatcher(app::channel_dispatcher_interface&);
